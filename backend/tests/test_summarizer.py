...
def test_default_summary_provider_uses_rule_based(monkeypatch) -> None:
...
def test_generate_summary_ignores_openai_provider_env(monkeypatch) -> None:
    monkeypatch.setattr(config, "ENV_FILE", config.PROJECT_ROOT / ".missing-test-env")
...
def test_generate_summary_ignores_ollama_provider_env(monkeypatch) -> None:
    monkeypatch.setattr(config, "ENV_FILE", config.PROJECT_ROOT / ".missing-test-env")
    ...
    assert "tldr" in summary
    assert any("notification" in item for item in summary["action_items"])
