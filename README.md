...
V2.5 adds a long-video processing mode backed by a lightweight local job store. `POST /api/videos/process-youtube-long` returns a queued job immediately (HTTP 202) and processes it in a FastAPI `BackgroundTasks` worker. Poll progress with `GET /api/jobs/{job_id}`.

V2.6 adds deterministic transcript quality analysis. Every processed transcript receives a `transcript_quality` level (`high` / `medium` / `low`), a list of human-readable `transcript_warnings`, and raw `quality_signals`. Markdown exports include a warning block when quality is below high.
...
## V2.6 Transcript Quality Warning

### Overview

Local STT (e.g. `faster-whisper`) can misrecognise product names, brand names, technical terms, and mixed-language Thai/English content. IzuMind now runs a lightweight, deterministic quality check on every transcript and surfaces the result in API responses and Markdown exports.

### What is added to API responses

Every response from `POST /api/videos/process` and `POST /api/videos/process-youtube` now includes:

| Field | Type | Description |
|---|---|---|
| `transcript_quality` | `"high"` / `"medium"` / `"low"` | Overall heuristic quality rating |
| `transcript_warnings` | `list[str]` | Human-readable warning messages |
| `transcript_quality_signals` | `dict` | Raw signals: char counts, ratios, flags |
...
```json
{
  ...
  "transcript_warnings": [
    "Transcript generated by local STT. Some words, names, or technical terms may be inaccurate.",
    "High proportion of Latin characters (38%) in a Thai transcript."
  ],
  "transcript_quality_signals": {
    "char_count": 1420,
    "unusual_character_ratio": 0.0012,
    "latin_character_ratio": 0.38,
    "thai_character_ratio": 0.41,
...
- Short or empty transcripts → `low`
- Unicode replacement characters (`\ufffd`) → `low` + warning
- High ratio of unusual characters → `low`
- For Thai transcripts: low Thai char ratio → `low`, high Latin ratio → `medium`
- Repeated phrases (possible STT loop) → `low` + warning
- `transcript_source == "local_stt"` → always adds a general STT warning

> This is a heuristic quality signal, not a guarantee. A high rating means the transcript looks structurally normal; it does not mean every word is correct.

### Markdown export

If `transcript_warnings` is non-empty or quality is below `high`, the exported Markdown includes a warning block immediately after the metadata header:

```markdown
> Transcript quality: **medium**
> ⚠️ หมายเหตุ: Transcript generated by local STT. Some words, names, or technical terms may be inaccurate.
```

High-quality transcripts with no warnings produce no extra noise in the Markdown output.

### Background jobs

Background job results (`process-youtube-long`) also store quality data in the video record, so Markdown export via `GET /api/videos/{video_id}/export/markdown` will include warnings even for videos processed asynchronously.
